...
Versioner = (function(_super) {
  ...
  Versioner.prototype.isVersioning = function() {
    return this._versioning;
  };
  Versioner.prototype.getSource = function() {
    return this._source;
  };
  ...
  Versioner.prototype.readFile = function(p_path) {
    ...
    try {
      this.running = true;
      this._source = data = fs.readFileSync(p_path, 'utf8');
    } catch (_error) {
      err = _error;
      this._versioning = false;
      this._source = null;
  ...
  Versioner.prototype.nextVersion = function(p_type) {
    ...
    this.notify('Current version: ' + release + '.' + build + '.' + bugfix, 'yellow');
    this.running = false;
  ...
  Versioner.prototype.destroy = function() {
    this.running = false;
    this._source = null;
    delete this._source;
    this._versioning = null;
    return delete this._versioning;
  };
  return Versioner;
})(EventDispatcher);
var TypeScriptCompiler;
TypeScriptCompiler = (function() {
  var Task;
  ...
  TypeScriptCompiler.prototype.addTask = function(name, task) {
    return this._tasks[this._tasks.length] = new Task(name, task);
  };
  TypeScriptCompiler.prototype.start = function(sourcePaths) {
    if (sourcePaths == null) {
      sourcePaths = [];
    }
    if (this._running) {
  ...
  TypeScriptCompiler.prototype.stop = function() {
    if (!this._running) {
      return;
    }
    return this._running = false;
  };
  TypeScriptCompiler.prototype.reset = function() {
    return 1;
  };
  TypeScriptCompiler.prototype.update = function(file) {
    if (this._running) {
      return this._runTasks();
    }
  };
  TypeScriptCompiler.prototype.remove = function(files) {
    return this._runTasks();
  };
  TypeScriptCompiler.prototype.runTasks = function(ugly, version) {
    if (ugly == null) {
      ugly = false;
    }
    if (version == null) {
  ...
  TypeScriptCompiler.prototype._runTasks = function(file, ugly, version) {
    var c, i, _results;
    if (file == null) {
      file = null;
    }
    ...
    while (i-- > 0) {
      c++;
      _results.push(this._tasks[i].output(this._sourcePaths, ugly, version));
    }
    return _results;
  };
  Task = (function() {
    ...
    Task.prototype.output = function(sourcePaths, ugly, version) {
      var dir, filename, i, l, out, p, psrc, versionResults, versioner, versioning, _results;
      if (ugly == null) {
        ugly = false;
      }
      if (version == null) {
      ...
      while (++i < l) {
        ...
        if (fs.existsSync(psrc)) {
          exec('vendors/typescript/bin/tsc --locale en --alwaysStrict --lib dom,es2015 --module amd --target es5 --outFile ' + p + ' ' + psrc, this._compiled.bind(this, ugly, versioning));
          break;
        } else {
          _results.push(void 0);
        }
    ...
    Task.prototype._shimJs = function(path, ugly) {
      var output;
      if (fs.existsSync(path)) {
        output = fs.readFileSync(path, 'utf8');
        if (output.indexOf(this.constructor.BUNDLE_PACK) === -1) {
          output = this.constructor.BUNDLE_PACK.replace('{{BUNDLE}}', output);
        }
        return output;
      }
      return '';
    };
    Task.prototype._compiled = function(ugly, versioning, error, output, errorMessage) {
      var e, out, p, results, t, versioner;
      if (/error TS/gi.test(output) || !!error) {
        Log.setStyle('red');
      ...
      p = path.resolve(this.data.output);
      out = this._shimJs(p);
      if (versioning != null ? versioning.results : void 0) {
      ...
      Log.setStyle('cyan');
      Log.println('In: ' + t + 's');
      return Notifier.notify('Compiler', 'TypeScript compilation completed!');
    };
...
CoffeeCompiler = (function() {
  var File, Task;
  CoffeeCompiler._ADD_NAMESPACE_FN = 'function __addNamespace(scope, obj){for(k in obj){if(!scope){eval(k + " = {};");scope = eval("(function(){return "+k+";})();");__addNamespace(scope, obj[k])}else if(!scope[k]){scope[k] = {};__addNamespace(scope[k], obj[k])};}};';
...
Main = (function() {
  function Main() {
    ...
    this.typeCompiler = new TypeScriptCompiler();
    this.typeCompiler.ugly = this.ugly;
  ...
  Main.prototype._fileChanged = function(e, files) {
    ...
    for (_i = 0, _len = files.length; _i < _len; _i++) {
      file = files[_i];
      if (/\.ts$/i.test(file)) {
        _results.push(this.typeCompiler.update(file));
  ...
  Main.prototype._fileRemoved = function(e, files) {
    files = [].concat(files);
    this.typeCompiler.remove(files);
  ...
  Main.prototype._onInput = function() {
    ...
    if (data) {
      ...
      switch (data) {
        case 'compile':
          this.typeCompiler.runTasks(false);
        ...
        case 'uglify':
          this.typeCompiler.runTasks(true);
        ...
        case 'build':
        case 'release':
          this.typeCompiler.runTasks(true, data);
  ...
  Main.prototype._reset = function() {
    var _ref;
    this.typeCompiler.stop();
    this.typeCompiler.reset();
  ...
  Main.prototype._buildFileChanged = function() {
    ...
    Log.println(this.sourcePaths);
    this.typeCompiler.start(this.sourcePaths);
  ...
  Main.prototype._parseTasks = function() {
    ...
    for (k in _ref) {
      ...
      src = task.src;
      if (/\.ts$/i.test(src)) {
        _results.push(this.typeCompiler.addTask(k, task));
