...
class SRTParser
	...
	@fromString:(p_string = null, p_milliseconds = false)->
		...
		while i < data.length
			items.push
				id: data[i].trim()
				startTime: if useMs then @_timeToMilliseconds(data[i + 1].trim()) else data[i + 1].trim()
				endTime: if useMs then @_timeToMilliseconds(data[i + 2].trim()) else data[i + 2].trim()
				text: data[i + 3].trim()
			i += 4
		items
