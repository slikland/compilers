    ...
    Task.prototype.output = function(sourcePaths, ugly) {
      ...
      while (++i < l) {
        ...
        if (fs.existsSync(psrc)) {
          exec('vendors/stylus/bin/stylus -u ./vendors/nib -c -I ' + sourcePaths.join(':') + ' ' + psrc + ' -o ' + p, this._compiled);
          break;
        } else {
          _results.push(void 0);
        }
  ...
  File = (function() {
    ...
    File.prototype.update = function() {
      ...
      if (fs.existsSync(this.file)) {
        this.content = fs.readFileSync(this.file, 'utf8');
        try {
          this.js = coffee.compile(this.content, {
            bare: true
          });
          return this.parseContent();
        } catch (_error) {
...
Main = (function() {
  ...
  Main.prototype._buildDocs = function(p_onInput) {
    var builder, json, options, opts, _ref, _ref1;
    if (p_onInput == null) {
      p_onInput = false;
    }
    ...
    this.buildFile.docs['attributesEmit'] = true;
  ...
  Main.prototype._buildFileChanged = function() {
    ...
    return setTimeout(this._ready, 1000);
  };
