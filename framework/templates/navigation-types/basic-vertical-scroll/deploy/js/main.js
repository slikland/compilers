...
SpriteSheetAnimation = (function(_super) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      ...
      if (this._useBackground) {
        fo.proportionalSource = {
          x: ((f.frame.x / (this._imageSize.w - f.frame.w)) * 100).toString() + '%',
          y: ((f.frame.y / (this._imageSize.h - f.frame.h)) * 100).toString() + '%',
  ...
  SpriteSheetAnimation.prototype._setupBackground = function(image) {
...
HomeView = (function(_super) {
  ...
  HomeView.prototype.create = function(evt) {
    ...
    data = this.content.spritesheets[0];
